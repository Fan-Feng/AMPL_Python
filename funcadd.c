...
#include "Python.h"  // Python.h need to be placed before funcadd.h

#include "asl/funcadd.h"	/* includes "stdio1.h" */

extern __declspec(dllimport) int Py_NoSiteFlag;

static real ginv(arglist *al)	/* generalized inverse of a single argument */
{
	real x = al->ra[0];
	x = x ? 1./x : 0.;
	if (al->derivs) {
...
Py_log(arglist *al){
	...
	pName = PyString_FromString("math");  // convert c string to Python string
	pModule = PyImport_Import(pName); //Import module
	Py_DECREF(pName); //Free memory

	if (pModule != NULL){
		pFunc = PyObject_GetAttrString(pModule,"log");

		if(pFunc && PyCallable_Check(pFunc)){
			//Read input from AMPL
			real x = al->ra[0];

			//Set this value to Python function's args
			pArgs = PyTuple_New(1);
 ...
 */
	...
	addfunc("Py_log", (rfunc)Py_log, 0, 1, 0);
